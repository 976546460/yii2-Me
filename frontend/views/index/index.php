	...
	<div class="nav w1210 bc mt10">
		...
		<div class="category fl"> <!-- 非首页，需要添加cat1类 -->
			...
			<?php foreach($goodsCategory as $k=>  $category):?>
				<div class="cat item1">
					<h3 class="">
						<?=\yii\helpers\Html::a($category->name,['index/list','cate_id'=>$category->id])?>
					</h3>
					<div class="cat_detail" style="display: none;">
						<?php  foreach ($category->children as $child): ?>
						<dl class="dl_1st">
							<dt>
								<?=\yii\helpers\Html::a($child->name,['index/list','cate_id'=>$child->id])?>
							</dt>
							<?php foreach ($child->children as $childes): ?>
							<dd>
								<?=\yii\helpers\Html::a($childes->name,['index/list','cate_id'=>$childes->id])?>
								<?php endforeach; ?>
							</dd>
						</dl>
						<?php endforeach; ?>
