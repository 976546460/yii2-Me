...
<!--    <a href="--><?//=\yii\helpers\Url::to(['article_category/index'])?><!--" class="btn btn-primary">分类列表</a >-->
    <tr class="danger <!--第一行背景色设置-->">
        <th class="col-lg-2">角色名称</th>
        <th class="col-lg-4">角色描述</th>
        <th class="col-lg-4">拥有权限</th>
        <th class="col-lg-1">操作</th>
    </tr>

    <?php
    $num=0; foreach ($models as $k=>$v):
        if($num%2==0){  ////////更换列表背景颜色
        ...
        <th class="col-lg-4">
            <?php foreach (\Yii::$app->authManager->getPermissionsByRole($v->name) as $permission) {
                echo $permission->description.', ';
            }

                ?></th>
        <th class="col-lg-1">
            <?=\yii\bootstrap\Html::a('修改',['rbac/edit-role','name'=>$v->name],['class'=>'btn btn-info btn-xs'])?>
            <?=\yii\bootstrap\Html::a('删除',['rbac/delete-role','name'=>$v->name],['class'=>'btn btn-danger btn-xs'])?>
        </th>
        </tr>
    <?php endforeach;  ?>
</table>
